...
import com.jaycodesx.mortgage.infrastructure.security.LeadResultTokenProperties;
...
@EnableConfigurationProperties(LeadResultTokenProperties.class)
public class LeadResultConsumer {
    ...
    private final LeadResultTokenProperties tokenProperties;
    private final QuoteMetricsService quoteMetricsService;

    public LeadResultConsumer(
            SqsClient sqsClient,
            ...
            LeadResultTokenProperties tokenProperties,
    ...
    public void poll() {
        String queueUrl = sqsClient.getQueueUrl(GetQueueUrlRequest.builder()
                .queueName(properties.leadResultQueueName())
                .build()).queueUrl();
        sqsClient.receiveMessage(ReceiveMessageRequest.builder()
                ...
                .forEach(message -> {
                    try {
                        LeadResultMessage payload = objectMapper.readValue(message.body(), LeadResultMessage.class);
                        if (!payload.hasSupportedSchemaVersion()) {
                            throw new IllegalArgumentException("Unsupported lead result schema version: " + payload.schemaVersion());
                        }
                        if (!messageDeduplicationService.firstReceipt("lead-result", payload.messageId())) {
                            quoteMetricsService.recordLeadResultMessageDeduped();
                            return;
                        }
                        ...
                        loanQuoteService.applyLeadResult(payload);
                    } catch (Exception ex) {
                        try {
                            deadLetterQueuePublisher.publish(properties.leadResultDlqName(), message.body(), ex.getMessage());
                            quoteMetricsService.recordLeadResultDlqPublish();
